...
class HistoryIngestionPipeline:
    ...
    @staticmethod
    def _estimate_confidence(data_source: DataSource, data: AirQualityData) -> float:
        base = {
            DataSource.LIVE: 0.92,
            DataSource.HISTORICAL: 0.90,
            DataSource.FORECAST: 0.75,
            DataSource.FALLBACK: 0.45,
        }[data_source]
        pollutant_count = len(data.pollutants.model_dump(exclude_none=True))
        boost = min(0.05, pollutant_count * 0.01)
        ...
        for attempt in range(1, self.max_retries + 2):
            try:
                ...
                snapshot = HistoricalSnapshotRecord(
                    ...
                    confidence=self._estimate_confidence(data_source, current_data),
                )
